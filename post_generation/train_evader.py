...
def build_prompt(prefix: str, source_text: str) -> str:
    return f"{prefix}\n\nInput:\n{source_text}\n\nRewrite:"
...
class HourlyEvasionEvalCallback(TrainerCallback):
    ...
    def _build_prompts(self) -> List[str]:
        return [build_prompt(self.instruction_prefix, s) for s in self.probe_sources]
...
def main() -> None:
    ...
    def preprocess_batch(batch: Dict[str, List[str]]) -> Dict[str, List[List[int]]]:
        prompts = [build_prompt(args.instruction_prefix, s) for s in batch["source_text"]]
